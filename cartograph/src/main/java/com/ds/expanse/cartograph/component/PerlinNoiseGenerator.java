...
//                image.setRGB(x, y, returnVal);
    ...
    public double perlin2(double x, double y) {
        double dx = x / 32;
        double dy = y / 32;
        return noise((dx * 3), (dy * 3));
    }

    private static double noise(double x, double y){
        int xi = (int)Math.floor(x); //bitwise conversion xi in RGB
        int yi = (int)Math.floor(y); //bitwise conversion yi in RGB
    ...
    public int perlin(double perlinValue, Function<Double, Integer> conversion){
        return conversion.apply(perlinValue);
    }

    public int perlin(int x, int y, Function<Double, Integer> conversion){
        return conversion.apply(perlin2(x, y));
    }
